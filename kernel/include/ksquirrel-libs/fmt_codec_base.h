...
class fmt_codec_base
{
	...
	virtual std::string     extension(const s32 bpp) = 0;
	...
	virtual s32	read_init(const std::string &file) = 0;
	...
	virtual s32	read_next_pass() = 0;

	// fmt_readscanline: read one scanline from file
	virtual s32	read_scanline(RGBA *scan) = 0;
	...
	virtual void    getwriteoptions(fmt_writeoptionsabs *) = 0;

	// fmt_write_init: init writing
	virtual s32     write_init(const std::string &file, const fmt_image &image, const fmt_writeoptions &opt) = 0;

	virtual s32	write_next() = 0;

	virtual s32	write_next_pass() = 0;

	// fmt_write_scanline: write scanline
	virtual s32	write_scanline(RGBA *scan) = 0;
